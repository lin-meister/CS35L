...
// Make the scene a global variable for easier access
scene_t scene;

// Make the float array global
float scaled_color[width][height][3];

void *multi_thread(void *thread_num) {
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
	...
	: pixel_dx;
	  ...
	  /* for every pixel */
	  for( int px=(tid*(width/nthreads)); px<(tid+1*(width/nthreads)); ++px)
	    {
...
main( int argc, char **argv )
{
  ...
  int * thread_args = (int*) malloc(sizeof(int) * nthreads);
  int ret;
  int t = 0;
  int s = 0;
  ...
  for (t = 0; t < nthreads; t++) {
    thread_args[t] = t;
    ret = pthread_create(&threads[t], NULL, multi_thread, (void*) &thread_args[t]);
    if (ret) {
  ...
  for (s = 0; s < nthreads; s++) {
    ...
    if (ret) {
      fprintf(stderr, "Could not join thread. Error Code: %d\n", ret);
    }
  }
